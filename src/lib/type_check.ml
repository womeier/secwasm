...
let type_checker (type label_type) (lat : label_type lattice) =
  (module struct
    ...
    (* ======= Lattice functions ============= *)
    let { leq; lub; str = lbl_to_str; bottom; top = _top } =
      lattice_functions lat
    ...
    let p_err3 msg l1 l2 l3 =
      let error_msg =
        Printf.sprintf msg (lbl_to_str l1) (lbl_to_str l2) (lbl_to_str l3)
      in
    ...
    let err_globalset1 t1 t2 =
      TypingError
        (Printf.sprintf "global.set src/ dst mismatch (src=%s, dst=%s)"
           (str_t t1) (str_t t2))
    ...
    let err_localset1 src dst =
      TypingError
        (Printf.sprintf "local.set src/ dst mismatch (src=%s, dst=%s)"
           (str_t src) (str_t dst))
    ...
    let err_store_val_i32 =
      TypingError "store expected value of type i32 to store"
    ...
    let rec check_instr ((gamma, c) : stack_of_stacks_type * context) i =
      ...
      | (st, pc) :: gamma' -> (
          ...
          | WI_Drop -> (
              ...
              | _ :: st' -> ((st', pc) :: gamma', c)
              | _ -> raise err_drop)
          | WI_Const _ -> (({ t = I32; lbl = pc } :: st, pc) :: gamma', c)
          | WI_BinOp _ -> (
              match st with
              | v1 :: v2 :: st' ->
                  assert (v1.t == v2.t);
          ...
          | WI_LocalSet idx -> (
              ...
              | { t = src; lbl = l } :: st' ->
                  ...
                  else if not (leq (lub l pc) l') then
                    raise (err_localset2 pc l l');
                  ((st', pc) :: gamma', c)
          ...
          | WI_Block block -> type_check_block c gamma block
    ...
    let type_check_function (ctx : context) f =
      ...
      let block =
        { btype = BlockType { params = []; result }; instrs = body }
      in
